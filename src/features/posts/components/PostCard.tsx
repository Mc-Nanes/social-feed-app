...
export function PostCard({
  ...
  const commentTextBeforeCaret = commentDraft.slice(0, commentCaretPosition)
  const commentMentionMatch = commentTextBeforeCaret.match(/(?:^|\s)@([^\s@]*)$/)
  const commentMentionQuery = commentMentionMatch?.[1]?.toLowerCase() ?? ''
  const commentMentionSuggestions = commentMentionMatch
    ? MOCK_MENTION_USERS.filter((username) =>
        username.toLowerCase().startsWith(commentMentionQuery),
      )
    : []
  ...
  function handleCommentMentionSelect(username: string) {
    if (!commentMentionMatch) {
      return
    }

    const mentionStartIndex = commentTextBeforeCaret.lastIndexOf('@')
    const beforeMention = commentDraft.slice(0, mentionStartIndex)
    const afterMention = commentDraft.slice(commentCaretPosition)
    const nextValue = `${beforeMention}@${username} ${afterMention}`

    setCommentDraft(nextValue)
    setCommentCaretPosition(mentionStartIndex + username.length + 2)
  }

  return (
    <article className="relative overflow-visible rounded-2xl border border-[#999999] bg-white">
      <header className="rounded-t-2xl flex items-start justify-between gap-4 bg-primary-500 px-5 py-5 text-white sm:px-6 sm:py-5">
          ...
          <div className="mt-4 space-y-4 rounded-2xl border border-[#999999] bg-white p-4">
            ...
            >
              <div className="relative flex-1">
                <input
                  className="h-11 w-full rounded-lg border border-black bg-white px-4 text-sm text-ink-950 outline-none transition-colors duration-200 focus:border-primary-500"
                  onChange={(event) => {
                    setCommentDraft(event.target.value)
                    setCommentCaretPosition(
                ...
                {commentMentionSuggestions.length > 0 ? (
                  <div className="absolute left-0 top-[calc(100%+8px)] z-20 w-full max-w-[220px] rounded-xl border border-[#999999] bg-white p-2 shadow-lg">
                    {commentMentionSuggestions.map((username) => (
                      <button
                        ...
                        key={`${post.id}-${username}`}
                        onClick={() => handleCommentMentionSelect(username)}
                        type="button"
                      >
                        @{username}
                      </button>
